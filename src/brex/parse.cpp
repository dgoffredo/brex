...
ParseError invalidCharacter(int byteOffset, char offendingCharacter) try {
    THROW_ERROR(ParseResult::INVALID_CHARACTER, byteOffset)
        << "Encountered the character \"" << offendingCharacter
...
std::unique_ptr<ParseTreeNode> parseOne(const std::string& input,
    ...
    // TODO: Need to work on diagnostics for empty alternation options.  Right
    //       now the error that is triggered is "unexpected ','" or "unexpected
    //       ';'".  It would be better to notice the case where you have ",,"
    //       or ",}".
...
void printDiagnostic(std::ostream&      stream,
    ...
    // TODO
    stream << "TODO: diagnostic when parsing: " << input << "\n"
           << error.message << "\n";
}
...
ParseResult parse(ParseTreeNode&     output,
    ...
    // `unique_ptr<ParseTreeNode>` (that is also never null), and then move the
    // referred-to `ParseTreeNode` into `output`.
    output = std::move(*parse(input, byteOffset));
